...
def get_pass(pass_type_id, serial_number):
    return get_object_or_404(
        Pass, pass_type_identifier=pass_type_id, serial_number=serial_number
    )
    ...
    def list(self, request, device_library_id, pass_type_id):
        ...
        if "passesUpdatedSince" in request.GET:
            passes = passes.filter(
                updated_at__gt=datetime.strptime(
        ...
        if passes:
            ...
            serial_numbers = [
                p.serial_number for p in passes.filter(updated_at=last_updated).all()
            ]
            response_data = {
...
class RegisterPassViewSet(viewsets.ViewSet):
    ...
    def create(self, request, device_library_id, pass_type_id, serial_number):
        ...
        if (
            ...
            != f"ApplePass {pass_.authentication_token}"
        ):
            return Response({}, status=status.HTTP_401_UNAUTHORIZED)
        registration = Registration.objects.filter(
            device_library_identifier=device_library_id, pazz=pass_,
        )
        if registration:
...
class LatestVersionViewSet(viewsets.ViewSet):
    ...
    def retrieve(self, request, pass_type_id, serial_number):
        ...
        if WALLETPASS_CONF["STORAGE_HTTP_REDIRECT"]:
            return Response(
                {}, status=status.HTTP_302_FOUND, headers={"Location": pass_.data.url}
            )
